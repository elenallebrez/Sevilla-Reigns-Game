...
The icons were recolored to match the project palette: cobalt blue for standard buttons and ivory for the burgundy exit button. The home screen uses generated animated frames first, then `assets/icons/static/` as the only menu fallback.
...
The event decision screen category bar uses restored legacy azulejo symbols from the original stat icons. The Pygame-ready files in `assets/icons/categories/` are transparent crops of the central symbol so the current vertical fill animation can use their alpha masks cleanly.

| Category | Asset | Source | Modifications |
| --- | --- | --- | --- |
| Tradición | `cross_cobalt.png` | `assets/images/religion_sil.png` | Cropped to the central cross and exported on a transparent square canvas. |
| Vecindario | `flamenca_cobalt.png` | `assets/images/people_sil.png` | Cropped to the flamenca silhouette, removed small frame artifacts, and exported on a transparent square canvas. |
| Dinero | `euro_cobalt.png` | `assets/images/money_sil.png` | Cropped to the central euro symbol and exported on a transparent square canvas. |
| Turismo | `suitcases_cobalt.png` | `assets/images/tourism_sil.png` | Cropped to the central suitcase group and exported on a transparent square canvas. |

The event image for `Cambio de recorrido` already exists in `assets/images/cambio_de_recorrido.png`, so no new event illustration was added for this screen.
...
License: [SIL Open Font License 1.1](https://openfontlicense.org/). Local license copies are stored as `assets/fonts/Cinzel-OFL.txt` and `assets/fonts/CormorantGaramond-OFL.txt`.
